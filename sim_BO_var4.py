...
batch_size=5
opt_num=batch_size*update_batch_times  #WSでは<=24
...
base_dir = f"../test_result/PREC_t=5_{Control_X_size}*{Control_Y_size}grids_FET={opt_num}_trials={trial_num}_{current_time}"
...
def init_val_intervation(num,Center_Control_X,Center_Control_Y,Control_Z,Control_MOMY):
    ...
    global org_file
    f.write(f"control Center_Control_X={Center_Control_X},Center_Control_Y={Center_Control_Y},Control_Z={Control_Z},Control_MOMY={Control_MOMY}\n")
    print(f"control Center_Control_X={Center_Control_X},Center_Control_Y={Center_Control_Y},Control_Z={Control_Z},Control_MOMY={Control_MOMY}\n")
    for pe in range(nofpe):
        output_file = f"000{num}/out-{Control_Var}.pe######.nc"
        sub_init_file = f"000{num}/init_d01_20070714-180000.000.pe######.nc"
        if num>=10:
            output_file = f"00{num}/out-{Control_Var}.pe######.nc"
...
def plot_PREC(trial_i, TEST_CTRL_prec_matrix):
    ...
    filename = os.path.join(base_dir, "PREC-heatmap", f"LontLat_seed={trial_i}.png")
...
with open(config_file_path,  "w") as f:
    ...
    # 制御する変数
    f.write("\n# 制御する変数\n")
    f.write(f"Control_Var = \"{Control_Var}\"\n")
    ...
    # 介入幅
    f.write("\n# 介入幅\n")
    f.write(f"Control_MOMY_low = {Control_MOMY_low}  # -30~30でエラー確認済み\n")
    f.write(f"Control_MOMY_high = {Control_MOMY_high}\n")
...
# 入力次元と最小値・最大値の定義
bounds = [ Integer(Control_X_low+2, Control_X_high-3),Integer(Control_Y_low+2, Control_Y_high-3) ,Integer(Control_Z_low, Control_Z_high),Real(Control_MOMY_low,Control_MOMY_high)]
...
with open(log_file, 'w') as f:
    for trial_i in range(trial_num):
        ...
        random_samples4 = [[random.uniform(Control_MOMY_low,Control_MOMY_high)] for _ in range(batch_size)]
