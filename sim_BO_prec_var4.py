...
base_dir = f"../test_result/prec_4var/{Control_Var_name}_t={Objective_T}_{Control_X_size}*{Control_Y_size}*{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
...
def predict(inputs, f):
    ...
    # 書き換えた初期値から runを回す run.d01.confで決められた時間先まで
    ...
    # 目的関数の値の計算
    result = calc_objective_func(f)
    return result

def calc_objective_func(f):
    ...
    print(f"t={Objective_T}:Sum of PREC in X[{Objective_X_low}, {Objective_X_high-1}], Y[{Objective_Y_low}, {Objective_Y_high-1}] = {result} [mm/h]\n")
    f.write(f"t={Objective_T}:Sum of PREC in X[{Objective_X_low}, {Objective_X_high-1}], Y[{Objective_Y_low}, {Objective_Y_high-1}] = {result} [mm/h]\n")
    return result

def init_val_intervation(num,LB_Control_X,LB_Control_Y,B_Control_Z,Control_Var, f):
    """
    与えられた制御変数の値（LB_Control_X~4）を用いて初期値を変更する。
    """
    
    global org_file
    f.write(f"control LB_Control_X={LB_Control_X},LB_Control_Y={LB_Control_Y},B_Control_Z={B_Control_Z},Control_Var={Control_Var}\n")
    print(f"control LB_Control_X={LB_Control_X},LB_Control_Y={LB_Control_Y},B_Control_Z={B_Control_Z},Control_Var={Control_Var}\n")
    for pe in range(nofpe):
        ...
        with netCDF4.Dataset(sub_init) as src, netCDF4.Dataset(output, "w") as dst:
            ...
            for name, variable in src.variables.items():
                ...
                if name == Control_Var_name:
                    var = src[name][:]
                    dst[name][:] = control_var_operation(var, pe, LB_Control_X, LB_Control_Y, Control_Y_size, Control_X_size, B_Control_Z, Control_Var)
                else:
                    dst[name][:] = src[name][:]
...
def sim(LB_Control_X,LB_Control_Y,B_Control_Z,Control_Var):
    """
...
def main():
    ...
    # 入力次元と最小値・最大値の定義
    bounds = [ Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size) ,Integer(Control_Z_low, Control_Z_high-Control_Z_size),Real(Control_Var_low,Control_Var_high)] # Int(2, 4) なら2, 3, 4からランダム
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a:
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            random_samples3 = [[random.randint(Control_Z_low, Control_Z_high-Control_Z_size) ] for _ in range(batch_size)]
