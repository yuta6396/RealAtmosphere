...
def sim(input1,input2,input3,input4):
    ...
    for j in range(70,80):
        for k in range(60,70):
            sum_gpy[j,k]+=dat[5,0,k,j]*TIME_INTERVAL #(y,x,z)=(time,z,y,x)
            sum_no[j,k]+=odat[5,0,k,j]*TIME_INTERVAL
            sum+=sum_gpy[j,k]
            no+=sum_no[j,k]
    return sum,no
...
bounds = [ Integer(2, 87),Integer(2, 87) ,Integer(0, 35),Real(-20,20)]

# 整数変数を追加
# bounds += [ Integer(0, 89)  ]  # 0~39の範囲
# bounds += [ Integer(0, 35) ]  # 0~97の範囲

opt = Optimizer(bounds, base_estimator="GP", acq_func="EI", random_state=2)
...
# 各範囲でランダム値を生成
random_samples1 = [[random.randint(2, 87) ] for _ in range(n)]
random_samples2 = [[random.randint(2, 87) ] for _ in range(n)]
random_samples3 = [[random.randint(0, 35) ] for _ in range(n)]
random_samples4 = [[random.uniform(-20, 20) ] for _ in range(n)]
...
no=0
gpy=0
gpy,no=sim(optimal_inputs[0],optimal_inputs[1],optimal_inputs[2],0)## please change
...
plt.savefig(f'result/BO_tohoku_restart_25grids_PREC_t=5_opt{opt_num}.png',dpi=300)
plt.close()
